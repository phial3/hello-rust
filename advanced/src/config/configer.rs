...
pub fn load_config() -> Result<Config, Box<dyn Error>> {
    ...
    println!("The command argument is {:?}", args);

    //1.find and read the config file.
    let config_path = std::env::args()
        .nth(1)
        .expect("Please at least give me the config file path.");
...
impl Config {
    ...
    pub fn load_db_cluster_config(&self) -> HashMap<String, DBClusterConfig> {
        let cluster_map: HashMap<String, DBClusterConfig> = self
            .cluster
            .iter()
            .map(|cc| (cc.id.clone(), cc.clone()))
            .collect();
        cluster_map
    }
    #[inline]
    pub fn load_db_node_config(&self) -> HashMap<String, DBNodeConfig> {
        let node_map: HashMap<String, DBNodeConfig> = self
            .node
            .iter()
            .map(|nc| (nc.id.clone(), nc.clone()))
            .collect();
        node_map
    }
} //end of impl Config
